...
import { testAttemptUpdateSchema } from "@/validations/test.schema";
...
export async function PATCH(
  ...
  try {
    ...
    const result = testAttemptUpdateSchema.safeParse(body);
    ...
    const { testAttemptId, ...updates } = result.data;

    const updatedTestAttempt = await testService.modifyTestAttempt(
      testAttemptId,
      updates
    );
    ...
    return NextResponse.json(
      {
        status: true,
        statusCode: 200,
        message: "Test attempted updated successfully",
        data: updatedTestAttempt,
      },
      HTTPSTATUS["200"]
    );
  } catch (error) {
